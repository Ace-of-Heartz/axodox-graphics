...
namespace Axodox::Graphics::D3D12
{
	...
	MeshDescription CreatePlane(float size, DirectX::XMUINT2 subdivisions)
	{
		...
		// Plain size? You mean subdivision size?
		if (subdivisions.x < 2 || subdivisions.y < 2) throw logic_error("Subdivision levels must be 2 at least!");
		if (subdivisions.x * subdivisions.y > (uint64_t)numeric_limits<uint32_t>::max() + 1) throw logic_error("Ran out of indices!");
